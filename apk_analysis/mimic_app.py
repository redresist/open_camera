#!/usr/bin/env python3
"""Mimic the EXACT app packet sequence to trigger camera P2P."""
import socket, struct, time

CAM_IP = "192.168.1.158"
CAM_UID_PREFIX = "DGOG"
CAM_SERIAL = 3247542  # HCAM03247542ABAMS
CAM_CHECK = "ABAMS"

SHUFFLE = bytes([
    ,
])
KEY = [0x69, 0x97, 0xcc, 0x19]

def enc(d):
    o, p = bytearray(), 0
    for b in d:
        i = (KEY[p & 3] + p) & 0xFF
        e = b ^ SHUFFLE[i]; o.append(e); p = e
    return bytes(o)

def dec(d):
    o, p = bytearray(), 0
    for b in d:
        i = (KEY[p & 3] + p) & 0xFF
        o.append(b ^ SHUFFLE[i]); p = b
    return bytes(o)

s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
s.bind(("0.0.0.0", 0))
s.settimeout(3)

# Step 1: Send LAN_SEARCH repeatedly (like the app does)
print("Step 1: LAN_SEARCH + recovery attempts...")
probe = enc(bytes([0xf1, 0x30, 0x00, 0x00]))
for _ in range(5):
    for port in [32108, 32100]:
        s.sendto(probe, (CAM_IP, port))
    time.sleep(0.1)

# Step 2: Send MSG_HELLO (like app does for discovery)
print("Step 2: MSG_HELLO...")
hello = enc(bytes([0xf1, 0x00, 0x00, 0x00]))
for _ in range(3):
    for port in [32108, 32100]:
        s.sendto(hello, (CAM_IP, port))
    time.sleep(0.1)

# Step 3: Listen for ANY response
print("\nStep 3: Listening for responses...")
deadline = time.time() + 10
found = {}
while time.time() < deadline:
    s.settimeout(max(0.1, deadline - time.time()))
    try:
        data, addr = s.recvfrom(4096)
    except socket.timeout:
        break
    if addr not in found:
        found[addr] = True
        magic = data[0]
        tag = "ENCR" if magic == 0x2c else "UNENCR" if magic == 0xf1 else f"0x{magic:02x}"
        print(f"  {addr}: {tag} {data.hex()[:40]}")
        d = dec(data) if magic == 0x2c else data
        if len(d) >= 4:
            op, pl = d[1], (d[2]<<8)|d[3]
            print(f"    op=0x{op:02x} len={pl}")

s.close()
print(f"\nDone. {len(found)} responders.")
