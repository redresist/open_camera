#!/usr/bin/env python3
"""Test Type B packet decryption with 10-byte PPPP header."""
import sys
import struct

sys.path.insert(0, r'E:\open_camera\apk_analysis')
from parse_pcap import parse_pcap

P2P_PE_TABLE = bytes([])

def derive_key(psk):
    k0 = sum(psk) & 0xFF
    k1 = (-k0) & 0xFF
    k2 = sum(b // 3 for b in psk) & 0xFF
    k3 = 0
    for b in psk:
        k3 ^= b
    return bytes([k0, k1, k2, k3])

def decrypt(key4, data):
    out = bytearray()
    prev = 0
    for c in data:
        k = key4[prev & 3]
        idx = (k + prev) & 0xFF
        ks = P2P_PE_TABLE[idx]
        out.append(c ^ ks)
        prev = c
    return bytes(out)

psks = [b'685FDC', b'camera', b'SSD@cs2-network.', b'cs2p2p', b'DGOG', b'admin', b'1234', b'scs2p2p', b'check_user', b'123456', b'888888', b'666666', b'CS2P2P', b'SHIX']

pkts = parse_pcap(r'E:\open_camera\apk_analysis\final_capture.pcap')

type_b_packets = []
for idx, pkt_data in enumerate(pkts):
    pkt = pkt_data[2]
    proto = struct.unpack_from('>H', pkt, 0)[0]
    ip_start = 20
    ip_header_len = (pkt[ip_start] & 0x0F) * 4
    udp_start = ip_start + ip_header_len
    payload = pkt[udp_start+8:]
    if len(payload) >= 10 and payload[2] == 0x68 and payload[3] == 0x04 and payload[4] == 0x54:
        type_b_packets.append((idx, payload))

print(f"Found {len(type_b_packets)} Type B packets")

# Decrypt first few Type B packets with relay key "5400"
key4_relay = derive_key(b'5400')

for pkt_idx, payload in type_b_packets[:5]:
    print(f"\n--- Packet {pkt_idx} ---")
    print(f"Header: {payload[:10].hex()}")
    size = struct.unpack('>H', payload[8:10])[0]
    print(f"SIZE: {size}, payload len: {len(payload)}")
    
    ciphertext = payload[10:]
    layer1 = decrypt(key4_relay, ciphertext)
    
    for psk in psks:
        key4 = derive_key(psk)
        plaintext = decrypt(key4, layer1)
        
        # Check for specific JSON markers
        if b'"pro"' in plaintext or b'"cmd"' in plaintext or b'check_user' in plaintext or b'admin' in plaintext:
            print(f"  FOUND with PSK={psk.decode()}!")
            print(f"  First 100 bytes: {plaintext[:100]}")
            # Find the JSON
            for marker in [b'"pro"', b'"cmd"']:
                if marker in plaintext:
                    pos = plaintext.index(marker)
                    print(f"  Marker '{marker.decode()}' at offset {pos}")
                    print(f"  Context: {plaintext[max(0,pos-30):pos+80]}")
                    break
            break
    else:
        # Check if any have {" just for curiosity
        for psk in psks:
            key4 = derive_key(psk)
            plaintext = decrypt(key4, layer1)
            if b'{"' in plaintext:
                pos = plaintext.index(b'{"')
                print(f"  {{\" found at offset {pos} with PSK={psk.decode()}")
                print(f"  Context: {plaintext[max(0,pos-20):pos+50]}")
                break
        else:
            print("  No promising JSON markers found")
