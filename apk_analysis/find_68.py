#!/usr/bin/env python3
"""Search for 0x68 in 0d-prefixed UDP payloads."""
import sys
import struct
import os
...
def find_0x68_in_0d_packets(path, network):
    ...
    for entry in udps:
        ...
        if len(p) < 2:
            continue
        # Look at 0d-prefixed packets
        if p[0] == 0x0d and p[1] != 0x00:
            # Also check 00-prefixed packets
            pass
        elif p[0] == 0x00 and p[1] == 0x0d:
            pass
        else:
            continue
        ...
        for off in range(min(len(p), 100)):
            if p[off] == 0x68:
                count += 1
                if count <= 20:
                    print(f"\n  Pkt {entry['pkt_idx']}: len={len(p)}, 0x68 at offset {off}")
...
for path, network in [
    ...
    if os.path.exists(path):
        find_0x68_in_0d_packets(path, network)
