#!/usr/bin/env python3
"""Probe DGOG camera at 192.168.1.158 with all protocols."""
import socket, struct, time

CAM_IP = "192.168.1.158"

SHUFFLE = bytes([
    ,
])
KEY = [0x69, 0x97, 0xcc, 0x19]

def enc(d):
    o, p = bytearray(), 0
    for b in d:
        i = (KEY[p & 3] + p) & 0xFF
        e = b ^ SHUFFLE[i]; o.append(e); p = e
    return bytes(o)

def dec(d):
    o, p = bytearray(), 0
    for b in d:
        i = (KEY[p & 3] + p) & 0xFF
        o.append(b ^ SHUFFLE[i]); p = b
    return bytes(o)

print(f"Probing {CAM_IP}...")
s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
s.bind(("0.0.0.0", 0))

# Send all probe types
probes = {
    "enc_lan":     enc(bytes([0xf1,0x30,0x00,0x00])),
    "enc_hello":   enc(bytes([0xf1,0x00,0x00,0x00])),
    "unenc_lan":   bytes([0xf1,0x30,0x00,0x00]),
    "unenc_hello": bytes([0xf1,0x00,0x00,0x00]),
}
for port in [32108, 32100]:
    for p in probes.values():
        s.sendto(p, (CAM_IP, port))

# Listen
found = set()
deadline = time.time() + 5
while time.time() < deadline:
    s.settimeout(max(0.1, deadline - time.time()))
    try:
        data, addr = s.recvfrom(4096)
    except socket.timeout:
        break
    if addr not in found:
        found.add(addr)
        magic = data[0]
        mode = "encrypted" if magic == 0x2c else "unencrypted" if magic == 0xf1 else f"0x{magic:02x}"
        print(f"  {addr[0]}:{addr[1]} [{mode}] {data.hex()[:40]}")
        d = dec(data) if magic == 0x2c else data
        if len(d) >= 4:
            op, pl = d[1], (d[2]<<8)|d[3]
            print(f"    -> op=0x{op:02x} len={pl} {d[4:4+pl].hex() if pl>0 else 'no payload'}")
            if op == 0x41 and pl >= 20:  # PUNCH
                pref = d[4:12].decode("ascii",errors="replace").rstrip("\x00")
                ser = int.from_bytes(d[12:16],"big")
                ck = d[16:24].decode("ascii",errors="replace").rstrip("\x00")
                print(f"    -> UID: {pref}-{ser:06d}-{ck}")

s.close()
print(f"Found {len(found)} responder(s)")
