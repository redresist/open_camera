#!/usr/bin/env python3
"""Probe camera while relay session is active - camera may have P2P port open."""
import socket, struct, time

CAM_IP = "192.168.1.158"
SHUFFLE = bytes([])
KEY = [0x69, 0x97, 0xcc, 0x19]

def enc(d):
    o, p = bytearray(), 0
    for b in d:
        i = (KEY[p & 3] + p) & 0xFF
        e = b ^ SHUFFLE[i]; o.append(e); p = e
    return bytes(o)

print("Probing camera while relay is active...")

# Try sending PUNCH request with the camera's UID
uid_payload = b"DGOG\x00\x00\x00\x00" + struct.pack(">I", 3247542) + b"ABAMS\x00\x00\x00"
punch_plain = bytes([0xf1, 0x41]) + struct.pack(">H", len(uid_payload)) + uid_payload
punch_enc = enc(punch_plain)

s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
s.bind(("0.0.0.0", 0))

# Try EVERY port from 32100-32110 and 22030-22040
ports = list(range(32100, 32111)) + list(range(22030, 22041))

seen = {}
for port in ports:
    for probe_name, probe in [("enc_punch", punch_enc), ("enc_lan", enc(bytes([0xf1,0x30,0x00,0x00])))]:
        s.sendto(probe, (CAM_IP, port))

# Listen
deadline = time.time() + 5
while time.time() < deadline:
    s.settimeout(0.1)
    try:
        data, addr = s.recvfrom(4096)
    except socket.timeout:
        continue
    if addr not in seen:
        seen[addr] = data
        print(f"  {addr}: {data[:30].hex()}")

s.close()
print(f"Found {len(seen)} unique responders")
