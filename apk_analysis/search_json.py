#!/usr/bin/env python3
"""Search for {\" inside full decrypted plaintexts."""
import sys
import struct

sys.path.insert(0, r'E:\open_camera\apk_analysis')
from parse_pcap import parse_pcap

P2P_PE_TABLE = bytes([])

def derive_key(psk):
    k0 = sum(psk) & 0xFF
    k1 = (-k0) & 0xFF
    k2 = sum(b // 3 for b in psk) & 0xFF
    k3 = 0
    for b in psk:
        k3 ^= b
    return bytes([k0, k1, k2, k3])

def decrypt(key4, data):
    out = bytearray()
    prev = 0
    for c in data:
        k = key4[prev & 3]
        idx = (k + prev) & 0xFF
        ks = P2P_PE_TABLE[idx]
        out.append(c ^ ks)
        prev = c
    return bytes(out)

def get_packet(path, pkt_idx):
    pkts = parse_pcap(path)
    pkt = pkts[pkt_idx][2]
    proto = struct.unpack_from('>H', pkt, 0)[0]
    ip_start = 20
    ip_header_len = (pkt[ip_start] & 0x0F) * 4
    udp_start = ip_start + ip_header_len
    return pkt[udp_start+8:]

def search_markers(payload, pppp_offset, relay_psk, layer1_psk):
    header = payload[pppp_offset:pppp_offset+8]
    
    # Try both relay key derivations
    for struct_name, rand0_idx, rand1_idx in [
        ("[SIZE][0x68][pad][rand0][rand1][CRC]", 4, 5),
        ("[0x68][pad][rand0][rand1][CRC][SIZE]", 2, 3),
    ]:
        rand0 = header[rand0_idx]
        rand1 = header[rand1_idx]
        psk_relay = f"{rand0:02X}{rand1:02X}".encode()
        key4_relay = derive_key(psk_relay)
        
        ciphertext = payload[pppp_offset+8:]
        layer1 = decrypt(key4_relay, ciphertext)
        
        key4 = derive_key(layer1_psk)
        plaintext = decrypt(key4, layer1)
        
        # Search for {" in full plaintext
        if b'{"' in plaintext:
            idx = plaintext.index(b'{"')
            print(f"\n  FOUND {{\" at offset {idx}!")
            print(f"  Structure: {struct_name}, relay={psk_relay}, layer1={layer1_psk}")
            print(f"  Context: {plaintext[max(0,idx-20):idx+50]}")
            return True
    return False

# Test the most promising candidates
payload = get_packet(r'E:\open_camera\apk_analysis\final_capture.pcap', 4357)
print("Testing pkt 4357:")
for psk in [b'685FDC', b'camera', b'check_user', b'admin', b'1234', b'DGOG', b'SHIX']:
    if search_markers(payload, 6, None, psk):
        break

payload = get_packet(r'E:\open_camera\apk_analysis\final_capture.pcap', 11)
print("\nTesting pkt 11:")
for psk in [b'685FDC', b'camera', b'check_user', b'admin', b'1234', b'DGOG', b'SHIX']:
    if search_markers(payload, 2, None, psk):
        break

payload = get_packet(r'E:\open_camera\apk_analysis\final_capture.pcap', 537)
print("\nTesting pkt 537:")
for psk in [b'685FDC', b'camera', b'check_user', b'admin', b'1234', b'DGOG', b'SHIX']:
    if search_markers(payload, 2, None, psk):
        break
