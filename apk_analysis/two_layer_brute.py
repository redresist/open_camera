#!/usr/bin/env python3
"""Try all PSK pairs as two-layer decryption on capture1 payloads."""
import sys
import struct

sys.path.insert(0, r'E:\open_camera\apk_analysis')
from parse_pcap import parse_pcap

P2P_PE_TABLE = bytes([])

def derive_key(psk):
    k0 = sum(psk) & 0xFF
    k1 = (-k0) & 0xFF
    k2 = sum(b // 3 for b in psk) & 0xFF
    k3 = 0
    for b in psk:
        k3 ^= b
    return bytes([k0, k1, k2, k3])

def decrypt(key4, data):
    out = bytearray()
    prev = 0
    for c in data:
        k = key4[prev & 3]
        idx = (k + prev) & 0xFF
        ks = P2P_PE_TABLE[idx]
        out.append(c ^ ks)
        prev = c
    return bytes(out)

PSK_CANDIDATES = [
    b"685FDC", b"camera", b"SSD@cs2-network.", b"cs2p2p", b"",
    b"DGOG", b"SHIX", b"admin", b"1234", b"HCAM03247542ABAMS",
    b"check_user", b"relay", b"p2p", b"DGOG-HCAM03247542ABAMS",
]

def is_printable(b):
    return 0x20 <= b <= 0x7e or b in (0x0a, 0x0d, 0x00)

pkts = parse_pcap(r'E:\open_camera\apk_analysis\capture1_new.pcap')

# Extract 111-byte 00-0d payloads
payloads = []
for i, (ts, ts_usec, pkt) in enumerate(pkts):
    if len(pkt) < 42:
        continue
    eth_type = struct.unpack_from('>H', pkt, 12)[0]
    if eth_type != 0x0800:
        continue
    ip_start = 14
    ip_header_len = (pkt[ip_start] & 0x0F) * 4
    if pkt[ip_start + 9] != 17:
        continue
    udp_start = ip_start + ip_header_len
    payload = pkt[udp_start+8:]
    
    if len(payload) == 111 and payload[:2] == b'\x00\x0d' and payload[8:10] == b'\x00\x69':
        seq = struct.unpack('>H', payload[2:4])[0]
        ct15 = payload[15:]
        ct16 = payload[16:]
        payloads.append({'pkt_idx': i, 'seq': seq, 'ct15': ct15, 'ct16': ct16})

print(f"Found {len(payloads)} payloads")
for p in payloads:
    print(f"  Pkt {p['pkt_idx']}: seq={p['seq']:04x}")

# Try all single-layer PSKs
print("\nTrying single-layer decryption...")
best_single = []
for entry in payloads:
    for psk in PSK_CANDIDATES:
        key4 = derive_key(psk)
        for ct, hdr_len in [(entry['ct15'], 15), (entry['ct16'], 16)]:
            pt = decrypt(key4, ct)
            printable = sum(1 for b in pt if is_printable(b))
            ratio = printable / len(pt)
            # Check for JSON markers
            markers_found = []
            for marker in [b'"pro"', b'"cmd"', b'check_user', b'admin', b'1234', b'"devmac"', b'"user"', b'"pwd"', b'{"']:
                if marker in pt:
                    markers_found.append(marker)
            if markers_found:
                best_single.append({
                    'pkt_idx': entry['pkt_idx'],
                    'seq': entry['seq'],
                    'hdr_len': hdr_len,
                    'psk': psk,
                    'ratio': ratio,
                    'markers': markers_found,
                    'pt': pt
                })

print(f"Single-layer matches: {len(best_single)}")
for m in best_single[:10]:
    print(f"  Pkt {m['pkt_idx']} seq={m['seq']:04x} hdr={m['hdr_len']} PSK={m['psk']} ratio={m['ratio']:.2%} markers={m['markers']}")
    print(f"    PT: {m['pt'][:80]}")

# Try all two-layer PSK pairs
print("\nTrying two-layer decryption...")
best_double = []
for entry in payloads:
    for psk1 in PSK_CANDIDATES:
        key4_1 = derive_key(psk1)
        for ct, hdr_len in [(entry['ct15'], 15), (entry['ct16'], 16)]:
            layer1 = decrypt(key4_1, ct)
            for psk2 in PSK_CANDIDATES:
                key4_2 = derive_key(psk2)
                pt = decrypt(key4_2, layer1)
                printable = sum(1 for b in pt if is_printable(b))
                ratio = printable / len(pt)
                markers_found = []
                for marker in [b'"pro"', b'"cmd"', b'check_user', b'admin', b'1234', b'"devmac"', b'"user"', b'"pwd"', b'{"']:
                    if marker in pt:
                        markers_found.append(marker)
                if markers_found:
                    best_double.append({
                        'pkt_idx': entry['pkt_idx'],
                        'seq': entry['seq'],
                        'hdr_len': hdr_len,
                        'psk_relay': psk1,
                        'psk_layer1': psk2,
                        'ratio': ratio,
                        'markers': markers_found,
                        'pt': pt
                    })

print(f"Two-layer matches: {len(best_double)}")
# Sort by ratio descending
best_double.sort(key=lambda x: -x['ratio'])
for m in best_double[:20]:
    print(f"  Pkt {m['pkt_idx']} seq={m['seq']:04x} hdr={m['hdr_len']} relay={m['psk_relay']} layer1={m['psk_layer1']} ratio={m['ratio']:.2%} markers={m['markers']}")
    print(f"    PT: {m['pt'][:80]}")
