#!/usr/bin/env python3
"""Test capture2_new.pcap pkt 262 with sub-packet at offset 13 (SIZE=57)."""
import sys
import struct

sys.path.insert(0, r'E:\open_camera\apk_analysis')
from parse_pcap import parse_pcap

P2P_PE_TABLE = bytes([])

def derive_key(psk):
    k0 = sum(psk) & 0xFF
    k1 = (-k0) & 0xFF
    k2 = sum(b // 3 for b in psk) & 0xFF
    k3 = 0
    for b in psk:
        k3 ^= b
    return bytes([k0, k1, k2, k3])

def decrypt(key4, data):
    out = bytearray()
    prev = 0
    for c in data:
        k = key4[prev & 3]
        idx = (k + prev) & 0xFF
        ks = P2P_PE_TABLE[idx]
        out.append(c ^ ks)
        prev = c
    return bytes(out)

pkts = parse_pcap(r'E:\open_camera\apk_analysis\capture2_new.pcap')

pkt262 = pkts[262][2]
ip_start = 14
ip_header_len = (pkt262[ip_start] & 0x0F) * 4
udp_start = ip_start + ip_header_len
payload = pkt262[udp_start+8:]

print(f"Payload length: {len(payload)}")
print(f"First 30 bytes: {payload[:30].hex()}")

# PPPP sub-packet at offset 13
pppp_offset = 13
header = payload[pppp_offset:pppp_offset+8]
print(f"PPPP header: {header.hex()}")

# Structure: [0x68][pad][rand0][rand1][CRC][SIZE]
rand0 = header[2]
rand1 = header[3]
size = struct.unpack('>H', header[6:8])[0]
print(f"rand0=0x{rand0:02x}, rand1=0x{rand1:02x}, SIZE={size}")

relay_key = f"{rand0:02X}{rand1:02X}".encode()
print(f"Relay key: {relay_key.decode()}")

ciphertext = payload[pppp_offset+8:pppp_offset+8+size]
print(f"Sub-packet ciphertext length: {len(ciphertext)}")
print(f"Sub-packet ciphertext: {ciphertext[:30].hex()}...")

key4_relay = derive_key(relay_key)
layer1 = decrypt(key4_relay, ciphertext)
print(f"Layer1 first 30 bytes: {layer1[:30].hex()}")

psks = [b'685FDC', b'camera', b'SSD@cs2-network.', b'cs2p2p', b'DGOG', b'admin', b'1234', b'scs2p2p', b'check_user', b'123456', b'888888', b'666666', b'CS2P2P', b'SHIX', b'', b'HCAM03247542ABAMS', b'DGOG-HCAM03247542ABAMS']

for psk in psks:
    key4 = derive_key(psk)
    plaintext = decrypt(key4, layer1)
    if b'{"' in plaintext or b'"pro"' in plaintext or b'"cmd"' in plaintext:
        print(f"\nFOUND JSON with PSK={psk.decode()}!")
        print(f"Plaintext: {plaintext[:80]}")
        break
    if plaintext[:2] == b'\xff\xd8':
        print(f"\nFOUND JPEG with PSK={psk.decode()}!")
        print(f"First 30 bytes: {plaintext[:30].hex()}")
        break
else:
    print("\nNo JSON or JPEG found with any PSK")
    # Show best printable
    best_psk = None
    best_ratio = 0
    for psk in psks:
        key4 = derive_key(psk)
        plaintext = decrypt(key4, layer1)
        printable = sum(1 for b in plaintext if 32 <= b < 127)
        ratio = printable / len(plaintext)
        if ratio > best_ratio:
            best_ratio = ratio
            best_psk = psk
    print(f"Best printable: {best_ratio:.2%} with PSK={best_psk.decode()}")
    key4 = derive_key(best_psk)
    plaintext = decrypt(key4, layer1)
    print(f"Best plaintext: {plaintext[:60].hex()}")
