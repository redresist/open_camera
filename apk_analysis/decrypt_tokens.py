#!/usr/bin/env python3
"""Decrypt discovery response tokens to extract UID/session data."""
import struct

shuffle_table = []

def create_psk_hash(psk):
    h = [0, 0, 0, 0]
    for b in psk.encode():
        h[0] = (h[0] + b) & 0xFF
        h[1] = (h[1] - b) & 0xFF
        h[2] = (h[2] + b // 3) & 0xFF
        h[3] = (h[3] ^ b) & 0xFF
    return h

def pppp_decrypt(psk, data):
    h = create_psk_hash(psk)
    result, prev = [], 0
    for b in data:
        idx = (h[prev & 3] + prev) & 0xFF
        result.append(b ^ shuffle_table[idx])
        prev = b
    return bytes(result)

# Decrypt different discovery responses
responses = [
    "2c8b821728b1c13191e638e0500ffafd74cd5e3b",
    "2c8b821728b10428a3409691fafd74cd5e3ba733",
    "2c8b821728b130d124384ca80bc290c35561358d",
]

print("=== Discovery Response Decryption ===")
for idx, resp_hex in enumerate(responses):
    enc = bytes.fromhex(resp_hex)
    dec = pppp_decrypt("camera", enc)
    print(f"\nResponse {idx+1}:")
    print(f"  Encrypted ({len(enc)}B): {resp_hex}")
    print(f"  Decrypted: {dec.hex()}")
    if dec[0] == 0xf1:
        opcode = dec[1]
        plen = struct.unpack(">H", dec[2:4])[0]
        payload = dec[4:4+plen]
        print(f"  opcode=0x{opcode:02x}, payload_len={plen}")
        print(f"  payload ({plen}B): {payload.hex()}")
        
        # UID format: prefix(8) + serial(4) + check(8) = 20 bytes
        if plen >= 20:
            prefix = payload[:8]
            serial_bytes = payload[8:12]
            serial = struct.unpack(">I", serial_bytes)[0]
            check = payload[12:20]
            print(f"  UID prefix: {prefix}")
            print(f"  Serial: {serial}")
            print(f"  Check: {check.hex()}")
        elif plen >= 16:
            print(f"  Data: {payload.hex()}")
