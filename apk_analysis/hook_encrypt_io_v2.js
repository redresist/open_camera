...
if (encryptPtr) {
    console.log("[*] P2P_Proprietary_Encrypt found at " + encryptPtr);
    Interceptor.attach(encryptPtr, {
        ...
        onLeave: function(retval) {
            if (this.len > 10 && this.len < 2000) {
                console.log("[ENCRYPT] PSK=" + this.psk + " len=" + this.len + " caller=" + this.caller);
                console.log("[ENCRYPT] IN:  " + toHex(this.inbuf, this.len));
                console.log("[ENCRYPT] ASC: " + toAscii(this.inbuf, this.len));
                console.log("[ENCRYPT] OUT: " + toHex(this.outbuf, this.len));
            }
        }
    });
} else {
    console.log("[!] Encrypt not found by mangled name, searching...");
    ...
    Interceptor.attach(writePtr, {
        onEnter: function(args) {
            ...
            if (len > 50 && len < 5000) {
                ...
                if (firstByte <= 0x0d || firstByte == 0x55) {
                    console.log("[WRITE] len=" + len + " fd=" + args[0] + " : " + toHex(buf, 40));
                }
            }
        }
    });
