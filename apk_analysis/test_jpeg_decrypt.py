#!/usr/bin/env python3
"""Test if decrypting Pattern3 packets with relay key '5400' yields ffd8."""
import sys
import struct

sys.path.insert(0, r'E:\open_camera\apk_analysis')
from parse_pcap import parse_pcap

P2P_PE_TABLE = bytes([])

def derive_key(psk):
    k0 = sum(psk) & 0xFF
    k1 = (-k0) & 0xFF
    k2 = sum(b // 3 for b in psk) & 0xFF
    k3 = 0
    for b in psk:
        k3 ^= b
    return bytes([k0, k1, k2, k3])

def decrypt(key4, data):
    out = bytearray()
    prev = 0
    for c in data:
        k = key4[prev & 3]
        idx = (k + prev) & 0xFF
        ks = P2P_PE_TABLE[idx]
        out.append(c ^ ks)
        prev = c
    return bytes(out)

psks = [b'685FDC', b'camera', b'SSD@cs2-network.', b'cs2p2p', b'DGOG', b'admin', b'1234', b'scs2p2p', b'check_user', b'123456', b'888888', b'666666', b'CS2P2P', b'SHIX']

pkts = parse_pcap(r'E:\open_camera\apk_analysis\final_capture.pcap')

key4_relay = derive_key(b'5400')

for idx, pkt_data in enumerate(pkts):
    pkt = pkt_data[2]
    if len(pkt) < 60:
        continue
    ip_start = 20
    ip_header_len = (pkt[ip_start] & 0x0F) * 4
    udp_start = ip_start + ip_header_len
    if len(pkt) < udp_start + 8:
        continue
    proto = pkt[ip_start + 9]
    if proto != 17:
        continue
    payload = pkt[udp_start+8:]
    if len(payload) > 7 and payload[6] == 0x68 and payload[7] == 0x04:
        ciphertext = payload[14:]
        layer1 = decrypt(key4_relay, ciphertext)
        
        found = False
        for psk in psks:
            key4 = derive_key(psk)
            plaintext = decrypt(key4, layer1)
            if plaintext[:2] == b'\xff\xd8':
                print(f"Pkt {idx}: JPEG found with PSK={psk.decode()}!")
                print(f"  First 50 bytes: {plaintext[:50].hex()}")
                found = True
                break
            if b'{"pro":"' in plaintext or b'"cmd":' in plaintext:
                print(f"Pkt {idx}: JSON found with PSK={psk.decode()}!")
                pos = plaintext.index(b'{"')
                print(f"  Context: {plaintext[max(0,pos-20):pos+80]}")
                found = True
                break
        
        if not found:
            # Check if any PSK produces printable ASCII
            best_psk = None
            best_ratio = 0
            for psk in psks:
                key4 = derive_key(psk)
                plaintext = decrypt(key4, layer1)
                printable = sum(1 for b in plaintext if 32 <= b < 127)
                ratio = printable / len(plaintext)
                if ratio > best_ratio:
                    best_ratio = ratio
                    best_psk = psk
            if best_ratio > 0.5:
                print(f"Pkt {idx}: best printable={best_ratio:.2%} with PSK={best_psk.decode()}")
                key4 = derive_key(best_psk)
                plaintext = decrypt(key4, layer1)
                print(f"  First 50 bytes: {plaintext[:50].hex()}")
