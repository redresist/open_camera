...
print(f"Section headers at offset {e_shoff:#x}, count {e_shnum}, strtab idx {e_shstrndx}")
...
for i in range(e_shnum):
    ...
    if name == '.symtab':
        symtab_off = s_offset
        symtab_num = s_size // s_entsize
        symtab_entsize = s_entsize
        print(f".symtab: offset={s_offset:#x}, num={symtab_num}, entsize={s_entsize}")
    if name == '.strtab':
        strtab_off = s_offset
        print(f".strtab: offset={s_offset:#x}, size={s_size:#x}")

if not symtab_off or not strtab_off:
    print("Could not find symtab/strtab")
    sys.exit(1)

# Search for TCPRelay symbols
...
for i in range(symtab_num):
    ...
    if st_name == 0:
        continue
    name = data[strtab_off + st_name : strtab_off + st_name + 100].split(b'\x00')[0]
    for t in targets:
