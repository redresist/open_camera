#!/usr/bin/env python3
"""Try different prev initializations to recover the key."""
import sys
import struct

sys.path.insert(0, r'E:\open_camera\apk_analysis')
from parse_pcap import parse_pcap

P2P_PE_TABLE = bytes([])

rev_table = {}
for i, v in enumerate(P2P_PE_TABLE):
    if v not in rev_table:
        rev_table[v] = []
    rev_table[v].append(i)

pkts = parse_pcap(r'E:\open_camera\apk_analysis\capture1_new.pcap')
pkt23 = pkts[23][2]
ip_start = 14
ip_header_len = (pkt23[ip_start] & 0x0F) * 4
udp_start = ip_start + ip_header_len
payload = pkt23[udp_start+8:]

c16 = payload[16:]
base_json = b'{"pro":"check_user","cmd":100,"devmac":"0000","user":"admin","pwd":"1234"}'
pt = base_json + b'\x00' * (96 - len(base_json))
pt16 = pt[:95]

# Compute keystream
ks = bytes(c ^ p for c, p in zip(c16, pt16))

def recover_key_with_init(ct, ks, prev_init):
    # ks[i] = P2P_PE_TABLE[(key4[prev & 3] + prev) & 0xFF]
    # prev = ct[i-1] for i > 0, but prev_init for i = 0
    
    candidates = []
    for k0 in rev_table.get(ks[0], []):
        # (key4[0] + prev_init) & 0xFF = idx
        # key4[0] = (idx - prev_init) & 0xFF
        k = (k0 - prev_init) & 0xFF
        candidates.append(([k], 1))
    
    if not candidates:
        return []
    
    for i in range(1, len(ks)):
        prev = ct[i-1]
        new_candidates = []
        for key_bytes, count in candidates:
            needed_byte_idx = i % 4
            if needed_byte_idx < len(key_bytes):
                k = key_bytes[needed_byte_idx]
                idx = (k + prev) & 0xFF
                if P2P_PE_TABLE[idx] == ks[i]:
                    new_candidates.append((key_bytes, count + 1))
            else:
                for idx in rev_table.get(ks[i], []):
                    k = (idx - prev) & 0xFF
                    new_key = key_bytes + [k]
                    new_candidates.append((new_key, count + 1))
        
        candidates = new_candidates
        if not candidates:
            return []
        
        max_count = max(c[1] for c in candidates)
        candidates = [c for c in candidates if c[1] == max_count]
        if len(candidates) > 100000:
            candidates = candidates[:100000]
    
    return candidates

# Try different prev_init values
print("Trying different prev_init values:")
for prev_init in [0, 0x68, 0x13, 0xde, 0xdc, 0x7c, 0xff, 0x80, 0x01, 0x69, 0x0d]:
    result = recover_key_with_init(c16, ks, prev_init)
    print(f"  prev_init=0x{prev_init:02x}: {len(result)} candidates, max_match={result[0][1] if result else 0}")
    if result and result[0][1] >= 10:
        for key_bytes, count in result[:5]:
            print(f"    key4={bytes(key_bytes).hex()}, matched={count}")

# Also try prev_init = key4[0] or other derived values
print("\nTrying prev_init based on key4 derivations:")
for psk in [b"685FDC", b"camera", b"SSD@cs2-network.", b"cs2p2p", b"DGOG", b"SHIX", b"admin", b"1234", b""]:
    k0 = sum(psk) & 0xFF
    k1 = (-k0) & 0xFF
    k2 = sum(b // 3 for b in psk) & 0xFF
    k3 = 0
    for b in psk:
        k3 ^= b
    key4 = bytes([k0, k1, k2, k3])
    for prev_init in [0, key4[0], key4[1], key4[2], key4[3], sum(key4) & 0xFF]:
        result = recover_key_with_init(c16, ks, prev_init)
        if result and result[0][1] >= 10:
            print(f"  PSK={psk}, prev_init=0x{prev_init:02x}: {len(result)} candidates, max_match={result[0][1]}")
            for key_bytes, count in result[:3]:
                print(f"    key4={bytes(key_bytes).hex()}, matched={count}")
