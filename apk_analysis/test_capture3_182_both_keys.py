#!/usr/bin/env python3
"""Test capture3 pkt182 with both relay key orderings."""
import sys
import struct

sys.path.insert(0, r'E:\open_camera\apk_analysis')
from parse_pcap import parse_pcap

P2P_PE_TABLE = bytes([])

def derive_key(psk):
    k0 = sum(psk) & 0xFF
    k1 = (-k0) & 0xFF
    k2 = sum(b // 3 for b in psk) & 0xFF
    k3 = 0
    for b in psk:
        k3 ^= b
    return bytes([k0, k1, k2, k3])

def decrypt(key4, data):
    out = bytearray()
    prev = 0
    for c in data:
        k = key4[prev & 3]
        idx = (k + prev) & 0xFF
        ks = P2P_PE_TABLE[idx]
        out.append(c ^ ks)
        prev = c
    return bytes(out)

pkts = parse_pcap(r'E:\open_camera\apk_analysis\capture3_new.pcap')

pkt182 = pkts[182][2]
ip_start = 14
ip_header_len = (pkt182[ip_start] & 0x0F) * 4
udp_start = ip_start + ip_header_len
payload = pkt182[udp_start+8:]

pppp_offset = 12
header = payload[pppp_offset:pppp_offset+8]
rand0 = header[4]
rand1 = header[5]

for relay_key in [f"{rand0:02X}{rand1:02X}".encode(), f"{rand1:02X}{rand0:02X}".encode()]:
    print(f"\n=== Testing relay key: {relay_key.decode()} ===")
    
    ciphertext = payload[pppp_offset+8:]
    key4_relay = derive_key(relay_key)
    layer1 = decrypt(key4_relay, ciphertext)
    
    # Test all PSKs
    psks = [b'685FDC', b'camera', b'SSD@cs2-network.', b'cs2p2p', b'DGOG', b'admin', b'1234', b'scs2p2p', b'check_user', b'123456', b'888888', b'666666', b'CS2P2P', b'SHIX']
    
    for psk in psks:
        key4 = derive_key(psk)
        plaintext = decrypt(key4, layer1)
        if plaintext[:2] == b'\xff\xd8':
            print(f"  FOUND JPEG with PSK={psk.decode()}!")
            print(f"  First 30 bytes: {plaintext[:30].hex()}")
            break
        if b'{"' in plaintext:
            pos = plaintext.index(b'{"')
            print(f"  Found {{\" at offset {pos} with PSK={psk.decode()}")
            print(f"  Context: {plaintext[max(0,pos-10):pos+40]}")
    else:
        print(f"  No JPEG or JSON found. Layer1 first 20 bytes: {layer1[:20].hex()}")
