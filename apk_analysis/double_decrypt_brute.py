# double_decrypt_brute.py
"""
Brute-force double encryption: first cs2p2p__P2P_Proprietary_Encrypt with session key,
then _TCPRelay_Proprietary_Encrypt with random 2-byte key.
"""

import sys
sys.path.insert(0, 'apk_analysis')
from parse_pcap import parse_pcap

P2P_PE_TABLE = bytes([
    ,
])

def derive_key(psk):
    k0 = sum(psk) & 0xFF
    k1 = (-k0) & 0xFF
    k2 = sum(b // 3 for b in psk) & 0xFF
    k3 = 0
    for b in psk:
        k3 ^= b
    return bytes([k0, k1, k2, k3])

def decrypt(key4, data):
    out = bytearray(len(data))
    prev = 0
    for i, c in enumerate(data):
        k = key4[prev & 3]
        idx = (k + prev) & 0xFF
        ks = P2P_PE_TABLE[idx]
        out[i] = c ^ ks
        prev = c
    return bytes(out)

def main():
    pkts = parse_pcap(r'E:\open_camera\apk_analysis\capture1_new.pcap')
    
    app_packets = []
    for i, (ts_sec, ts_usec, pkt) in enumerate(pkts):
        if len(pkt) < 54: continue
        ip_off = 14
        ip_len = (pkt[ip_off] & 0x0F) * 4
        tcp_off = ip_off + ip_len
        tcp_hdr_len = ((pkt[tcp_off + 12] >> 4) & 0x0F) * 4
        payload = pkt[tcp_off + tcp_hdr_len:]
        if payload[:4] == b'\x8c\x13\xea\xb2':
            encrypted = payload[8+15:]
            app_packets.append((i, encrypted))
    
    # First-layer PSK candidates
    first_layer_psks = [
        b'685FDC',           # cloud_key from logcat
        b'685F',             # first 4 chars of cloud_key
        b'685FDC00',         # padded
        b'363835464443',     # ASCII hex of cloud_key
        b'vstarcam2019',     # known VStarcam PSK
        b'vstarcam2018',     # known VStarcam PSK
        b'',                 # empty PSK
        b'camera',           # common default
        b'1234',             # simple default
        b'admin',            # simple default
    ]
    
    target_substrings = [b'check_user', b'get_product', b'stream', b'"pro"', b'"cmd"', 
                         b'"devmac"', b'"user"', b'"pwd"', b'admin', b'1234']
    
    print(f"Testing {len(app_packets)} app->relay packets")
    print(f"First-layer PSKs: {[p.decode() for p in first_layer_psks]}")
    
    for pkt_idx, ciphertext in app_packets:
        print(f"\n=== Packet {pkt_idx} (ciphertext len={len(ciphertext)}) ===")
        found_any = False
        
        for b0 in range(256):
            for b1 in range(256):
                psk2 = bytes([b0, b1]).hex().upper().encode()
                key4_relay = derive_key(psk2)
                intermediate = decrypt(key4_relay, ciphertext)
                
                for psk1 in first_layer_psks:
                    key4_p2p = derive_key(psk1)
                    plaintext = decrypt(key4_p2p, intermediate)
                    
                    score = sum(1 for s in target_substrings if s in plaintext)
                    if score > 0:
                        found_any = True
                        print(f"RELAY_KEY={psk2.decode()} FIRST_PSK={psk1.decode()} score={score}")
                        print(f"  Plaintext: {plaintext[:80]}")
        
        if not found_any:
            print("No matches found")

if __name__ == '__main__':
    main()
