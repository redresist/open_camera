...
for fname in ['capture1_new.pcap', 'capture2_new.pcap', 'capture3_new.pcap']:
    ...
    for p in pkts:
        pl = p['payload']
        if pl and pl[0] == 0x00 and len(pl) > 15:
            print(f"  Pkt {p['num']}: {p['src']} -> {p['dst']}  {p['len']}B  {pl[:min(40,p['len'])].hex()}")
    
    for p in pkts:
        pl = p['payload']
        if pl and pl[0] in [0x0b, 0x01]:
            print(f"  Pkt {p['num']}: {p['src']} -> {p['dst']}  CTRL 0x{pl[0]:02x} {p['len']}B  {pl[:10].hex()}")
    
    for p in pkts:
        pl = p['payload']
        if pl and pl[0] == 0x2c and len(pl) == 20:
            print(f"  Pkt {p['num']}: {p['src']} -> {p['dst']}  DISCOVERY 20B  {pl.hex()}")
