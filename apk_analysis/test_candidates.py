#!/usr/bin/env python3
"""Test decryption on most promising 0x68 candidates."""
import sys
import struct

sys.path.insert(0, r'E:\open_camera\apk_analysis')
from parse_pcap import parse_pcap

P2P_PE_TABLE = bytes([])

def derive_key(psk):
    k0 = sum(psk) & 0xFF
    k1 = (-k0) & 0xFF
    k2 = sum(b // 3 for b in psk) & 0xFF
    k3 = 0
    for b in psk:
        k3 ^= b
    return bytes([k0, k1, k2, k3])

def decrypt(key4, data):
    out = bytearray()
    prev = 0
    for c in data:
        k = key4[prev & 3]
        idx = (k + prev) & 0xFF
        ks = P2P_PE_TABLE[idx]
        out.append(c ^ ks)
        prev = c
    return bytes(out)

psks = [b'685FDC', b'camera', b'SSD@cs2-network.', b'cs2p2p', b'DGOG', b'admin', b'1234', b'scs2p2p', b'check_user', b'123456', b'888888', b'666666', b'CS2P2P', b'SHIX']

def get_payload(path, pkt_idx):
    pkts = parse_pcap(path)
    pkt = pkts[pkt_idx][2]
    
    # Determine network type
    with open(path, 'rb') as f:
        header = f.read(24)
    network = struct.unpack_from('<I', header, 20)[0]
    
    if network == 1:
        eth_type = struct.unpack_from('>H', pkt, 12)[0]
        if eth_type == 0x0800:
            ip_start = 14
        elif eth_type == 0x8100:
            real_eth_type = struct.unpack_from('>H', pkt, 16)[0]
            if real_eth_type == 0x0800:
                ip_start = 18
            else:
                return None
        else:
            return None
    elif network == 276:
        proto = struct.unpack_from('>H', pkt, 0)[0]
        if proto != 0x0800:
            return None
        ip_start = 20
    else:
        return None
    
    ip_header_len = (pkt[ip_start] & 0x0F) * 4
    udp_start = ip_start + ip_header_len
    return pkt[udp_start+8:]

def test_candidate(path, pkt_idx, offset, structure, rand0, rand1):
    payload = get_payload(path, pkt_idx)
    if payload is None:
        return
    
    relay_key = f"{rand0:02X}{rand1:02X}".encode()
    key4_relay = derive_key(relay_key)
    
    if structure == '[0x68][pad][rand0][rand1][CRC][SIZE]':
        ciphertext = payload[offset+8:]
    else:
        ciphertext = payload[offset+6:]
    
    layer1 = decrypt(key4_relay, ciphertext)
    
    print(f"\nTesting {path} pkt {pkt_idx}, offset={offset}, structure={structure}")
    print(f"Relay key: {relay_key.decode()}")
    print(f"Ciphertext length: {len(ciphertext)}")
    
    for psk in psks:
        key4 = derive_key(psk)
        plaintext = decrypt(key4, layer1)
        
        # Check for specific patterns
        if b'"pro"' in plaintext or b'"cmd"' in plaintext or b'check_user' in plaintext:
            print(f"  FOUND JSON with PSK={psk.decode()}!")
            pos = plaintext.index(b'{"')
            print(f"  Context: {plaintext[max(0,pos-20):pos+80]}")
            return True
        if plaintext[:2] == b'\xff\xd8':
            print(f"  FOUND JPEG with PSK={psk.decode()}!")
            return True
        if b'{"' in plaintext:
            pos = plaintext.index(b'{"')
            print(f"  Found {{\" at offset {pos} with PSK={psk.decode()}")
            print(f"  Context: {plaintext[max(0,pos-20):pos+50]}")
    
    # Show best attempt
    best_psk = None
    best_ratio = 0
    for psk in psks:
        key4 = derive_key(psk)
        plaintext = decrypt(key4, layer1)
        printable = sum(1 for b in plaintext if 32 <= b < 127)
        ratio = printable / len(plaintext)
        if ratio > best_ratio:
            best_ratio = ratio
            best_psk = psk
    
    if best_ratio > 0.3:
        print(f"  Best printable: {best_ratio:.2%} with PSK={best_psk.decode()}")
        key4 = derive_key(best_psk)
        plaintext = decrypt(key4, layer1)
        print(f"  First 60 bytes: {plaintext[:60].hex()}")
    
    return False

# Test most promising candidates from comprehensive search
candidates = [
    (r'E:\open_camera\apk_analysis\relay.pcap', 15, 29, '[0x68][pad][rand0][rand1][CRC][SIZE]', 0xa3, 0x34),
    (r'E:\open_camera\apk_analysis\relay.pcap', 77, 365, '[0x68][pad][rand0][rand1][CRC][SIZE]', 0x94, 0x52),
    (r'E:\open_camera\apk_analysis\handshake.pcap', 12, 201, '[0x68][pad][rand0][rand1][CRC][SIZE]', 0xad, 0x23),
    (r'E:\open_camera\apk_analysis\full_handshake.pcap', 11, 19, '[SIZE][0x68][pad][rand0][rand1][CRC]', 0xd3, 0x35),
]

for c in candidates:
    test_candidate(*c)
