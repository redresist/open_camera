#!/usr/bin/env python3
"""
Test PPPP decryption on captured relay credential payloads.
Algorithm from pppp-dissector (magicus/pppp-dissector).
"""
import struct

shuffle_table = [
    ,
]

def create_psk_hash(psk):
    h = [0, 0, 0, 0]
    for b in psk.encode():
        h[0] = (h[0] + b) & 0xFF
        h[1] = (h[1] - b) & 0xFF
        h[2] = (h[2] + b // 3) & 0xFF
        h[3] = (h[3] ^ b) & 0xFF
    return h

def pppp_encrypt(psk, data):
    h = create_psk_hash(psk)
    result = []
    prev = 0
    for b in data:
        idx = (h[prev & 3] + prev) & 0xFF
        key = shuffle_table[idx]
        enc = b ^ key
        result.append(enc)
        prev = enc
    return bytes(result)

def pppp_decrypt(psk, data):
    h = create_psk_hash(psk)
    result = []
    prev = 0
    for b in data:
        idx = (h[prev & 3] + prev) & 0xFF
        key = shuffle_table[idx]
        dec = b ^ key
        result.append(dec)
        prev = b
    return bytes(result)

# Verify magic bytes
print("=== Magic Byte Verification ===")
for psk in ['camera', 'SHIX', 'SSD@cs2-network.']:
    mb = pppp_encrypt(psk, bytes([0xf1]))[0]
    print(f"  {psk:20s} -> 0x{mb:02x} (expected: camera=0x2c, SHIX=0x9f, SSD@cs2-network.=0xb1)")

# Try decrypting known PPPP packets from captures
from scapy.all import rdpcap, UDP, IP, raw
import os
import sys

for pcap_file in ['final_capture.pcap', 'startup2.pcap', 'relay.pcap']:
    path = os.path.join(os.path.dirname(__file__) or '.', pcap_file)
    if not os.path.exists(path):
        continue
    print(f"\n=== {pcap_file} ===")
    try:
        pkts = rdpcap(path)
        for i, pkt in enumerate(pkts):
            if UDP in pkt and raw in pkt:
                data = bytes(pkt[raw])
                # Check first byte against known magic bytes
                magic = data[0]
                psk = None
                if magic == 0xf1:
                    print(f"  Pkt {i}: UNENCRYPTED f1, len={len(data)}")
                    if len(data) >= 4:
                        opcode = data[1]
                        plen = struct.unpack('>H', data[2:4])[0]
                        print(f"    opcode=0x{opcode:02x}, payload_len={plen}")
                        if plen > 0 and 4 + plen <= len(data):
                            payload = data[4:4+plen]
                            print(f"    payload: {payload.hex()[:80]}")
                elif magic == 0x2c:
                    psk = "camera"
                elif magic == 0x9f:
                    psk = "SHIX"
                elif magic == 0xb1:
                    psk = "SSD@cs2-network."
                elif magic == 0x00 and len(data) > 1:
                    # Could be relay command channel
                    cmd_flags = struct.unpack('<I', data[:4])[0]
                    if cmd_flags in [0x060AA080, 0x00000000]:
                        print(f"  Pkt {i}: Possible command channel, len={len(data)}")
                        print(f"    raw: {data[:60].hex()}")
                        # The encrypted data in command channel starts after the header
                        # Header: flags(4) + cmd_len_le(4) + data
                        # But our 00-prefixed packets may be different
                else:
                    continue  # skip non-PPPP packets

                if psk:
                    decrypted = pppp_decrypt(psk, data)
                    if decrypted[0] == 0xf1:
                        opcode = decrypted[1]
                        plen = struct.unpack('>H', decrypted[2:4])[0]
                        print(f"  Pkt {i}: PSK={psk}, opcode=0x{opcode:02x}, len={len(data)}")
                        if plen > 0 and 4 + plen <= len(decrypted):
                            payload = decrypted[4:4+plen]
                            print(f"    payload: {payload.hex()[:80]}")
                            # Try to decode as ASCII
                            try:
                                text = payload.decode('ascii', errors='replace')
                                if any(c.isprintable() for c in text):
                                    print(f"    text: {text[:100]}")
                            except:
                                pass
                    else:
                        print(f"  Pkt {i}: PSK={psk} decrypted but no f1 magic: {decrypted[:20].hex()}")

    except Exception as e:
        print(f"  Error: {e}")
