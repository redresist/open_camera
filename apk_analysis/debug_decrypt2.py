# debug_decrypt2.py
import sys
sys.path.insert(0, 'apk_analysis')
from parse_pcap import parse_pcap

P2P_PE_TABLE = bytes([
    ,
])

def derive_key(psk):
    k0 = sum(psk) & 0xFF
    k1 = (-k0) & 0xFF
    k2 = sum(b // 3 for b in psk) & 0xFF
    k3 = 0
    for b in psk:
        k3 ^= b
    return bytes([k0, k1, k2, k3])

def decrypt(key4, data):
    out = bytearray(len(data))
    prev = 0
    for i, c in enumerate(data):
        k = key4[prev & 3]
        idx = (k + prev) & 0xFF
        ks = P2P_PE_TABLE[idx]
        out[i] = c ^ ks
        prev = c
    return bytes(out)

pkts = parse_pcap(r'E:\open_camera\apk_analysis\capture1_new.pcap')

key_bytes = bytes.fromhex('bc96')
psk = key_bytes.hex().upper().encode()
key4 = derive_key(psk)
print(f'Key={key_bytes.hex()} PSK={psk.decode()} key4={key4.hex()}')

for i, (ts_sec, ts_usec, pkt) in enumerate(pkts):
    if len(pkt) < 54:
        continue
    ip_off = 14
    ip_len = (pkt[ip_off] & 0x0F) * 4
    tcp_off = ip_off + ip_len
    tcp_hdr_len = ((pkt[tcp_off + 12] >> 4) & 0x0F) * 4
    payload = pkt[tcp_off + tcp_hdr_len:]
    if payload[:4] == b'\xea\xb2\x8c\x13':
        for header_len in range(1, 25):
            encrypted = payload[8+header_len:]
            if len(encrypted) == 0:
                continue
            decrypted = decrypt(key4, encrypted)
            if decrypted[0:1] == b'{' or decrypted[0:1] == b'[':
                print(f'\nPkt {i} header_len={header_len}:')
                print(f'  first_enc={encrypted[0]:#x} -> decrypted[0]={decrypted[0]:#x} ({chr(decrypted[0])})')
                print(f'  Bytes around header_end: {payload[8+header_len-2:8+header_len+5].hex()}')
                print(f'  Decrypted: {decrypted[:60]}')
