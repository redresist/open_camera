#!/usr/bin/env python3
"""
Derive relay credential PSK hash from known-plaintext attack.
Assumes plaintext is JSON padded with nulls to 96 bytes.
"""

SHUFFLE = bytes([
    ,
])

SHUFFLE_INVERSE = [0] * 256
for i, v in enumerate(SHUFFLE):
    SHUFFLE_INVERSE[v] = i

# Known encrypted credential payloads (96 bytes each, from captures)
SESSIONS = {
    'capture1_a6e7_pkt0': bytes.fromhex('7638b996d6c23f9db9f64082cbcdad0002fd488e2c244d9562'),
    'capture2_a6e7_pkt0': bytes.fromhex('b945b2c55fbe2e3215d8de3ef75cebcdf9bc35f59e0e439b95'),
    'capture3_90d1_pkt0': bytes.fromhex('e96af35378d8ed637cef04a2e9783598f0acd4ac0d2792da25'),
}

# Note: these are only the first 23 bytes shown. We need the full 96 bytes.
# Let me extract from pcaps properly.
