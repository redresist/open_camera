# comprehensive_brute.py
"""
Comprehensive brute-force of all 65536 relay keys against all payloads,
looking for known plaintext substrings.
"""

import sys
sys.path.insert(0, 'apk_analysis')
from parse_pcap import parse_pcap

P2P_PE_TABLE = bytes([
    ,
])

def derive_key(psk):
    k0 = sum(psk) & 0xFF
    k1 = (-k0) & 0xFF
    k2 = sum(b // 3 for b in psk) & 0xFF
    k3 = 0
    for b in psk:
        k3 ^= b
    return bytes([k0, k1, k2, k3])

def decrypt(key4, data):
    out = bytearray(len(data))
    prev = 0
    for i, c in enumerate(data):
        k = key4[prev & 3]
        idx = (k + prev) & 0xFF
        ks = P2P_PE_TABLE[idx]
        out[i] = c ^ ks
        prev = c
    return bytes(out)

def main():
    pkts = parse_pcap(r'E:\open_camera\apk_analysis\capture1_new.pcap')
    
    # Extract all app->relay and relay->app payloads
    app_packets = []
    relay_packets = []
    for i, (ts_sec, ts_usec, pkt) in enumerate(pkts):
        if len(pkt) < 54:
            continue
        ip_off = 14
        ip_len = (pkt[ip_off] & 0x0F) * 4
        tcp_off = ip_off + ip_len
        tcp_hdr_len = ((pkt[tcp_off + 12] >> 4) & 0x0F) * 4
        payload = pkt[tcp_off + tcp_hdr_len:]
        if len(payload) < 30:
            continue
        if payload[:4] == b'\x8c\x13\xea\xb2':
            app_packets.append((i, payload))
        elif payload[:4] == b'\xea\xb2\x8c\x13':
            relay_packets.append((i, payload))
    
    print(f"App->relay packets: {[p[0] for p in app_packets]}")
    print(f"Relay->app packets: {[p[0] for p in relay_packets]}")
    
    # Known substrings to search for
    target_substrings = [b'check_user', b'get_product', b'stream', b'"pro"', b'"cmd"', 
                         b'"devmac"', b'"user"', b'"pwd"', b'admin', b'1234', b'"admin"']
    
    # For each app->relay packet, try all 65536 keys
    for pkt_idx, payload in app_packets:
        print(f"\n=== Brute-forcing packet {pkt_idx} (app->relay, len={len(payload)}) ===")
        best_score = 0
        best_results = []
        
        # Extract encrypted portion (skip 8-byte tunnel + 15-byte relay header)
        encrypted = payload[8+15:]
        print(f"Encrypted payload length: {len(encrypted)}")
        
        for b0 in range(256):
            for b1 in range(256):
                psk = bytes([b0, b1]).hex().upper().encode()
                key4 = derive_key(psk)
                decrypted = decrypt(key4, encrypted)
                
                score = sum(1 for s in target_substrings if s in decrypted)
                if score > best_score:
                    best_score = score
                    best_results = [(b0, b1, psk.decode(), score, decrypted)]
                elif score == best_score and score > 0:
                    best_results.append((b0, b1, psk.decode(), score, decrypted))
        
        print(f"Best score: {best_score}")
        for r in best_results[:5]:
            print(f"  Key={r[0]:02x}{r[1]:02x} PSK={r[2]} score={r[3]}")
            print(f"    Decrypted: {r[4][:80]}")
    
    # For relay->app packets, look for JSON responses
    print("\n=== Brute-forcing relay->app packets for JSON ===")
    for pkt_idx, payload in relay_packets:
        if len(payload) < 50:
            continue
        print(f"\nPacket {pkt_idx} (len={len(payload)}):")
        
        best_score = 0
        best_results = []
        
        for header_len in range(10, 25):
            encrypted = payload[8+header_len:]
            if len(encrypted) < 20:
                continue
            
            for b0 in range(256):
                for b1 in range(256):
                    psk = bytes([b0, b1]).hex().upper().encode()
                    key4 = derive_key(psk)
                    decrypted = decrypt(key4, encrypted)
                    
                    # Look for JSON indicators
                    score = 0
                    if decrypted[0:1] == b'{':
                        score += 2
                    if b'"cmd"' in decrypted:
                        score += 2
                    if b'"admin"' in decrypted:
                        score += 2
                    if b'"result"' in decrypted:
                        score += 1
                    # Check if most bytes are printable ASCII
                    printable = sum(1 for b in decrypted if 32 <= b <= 126 or b in (9, 10, 13))
                    if printable > len(decrypted) * 0.8:
                        score += 1
                    
                    if score > best_score:
                        best_score = score
                        best_results = [(header_len, b0, b1, psk.decode(), score, decrypted)]
                    elif score == best_score and score > 0:
                        best_results.append((header_len, b0, b1, psk.decode(), score, decrypted))
        
        print(f"  Best score: {best_score}")
        for r in best_results[:5]:
            print(f"    header_len={r[0]} Key={r[1]:02x}{r[2]:02x} PSK={r[3]} score={r[4]}")
            print(f"      Decrypted: {r[5][:100]}")

if __name__ == '__main__':
    main()
