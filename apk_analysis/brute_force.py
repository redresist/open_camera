#!/usr/bin/env python3
"""Brute force first-layer PSK on 00-0d relay packets."""
import sys
import struct

sys.path.insert(0, r'E:\open_camera\apk_analysis')
from parse_pcap import parse_pcap

P2P_PE_TABLE = bytes([])

def derive_key(psk):
    k0 = sum(psk) & 0xFF
    k1 = (-k0) & 0xFF
    k2 = sum(b // 3 for b in psk) & 0xFF
    k3 = 0
    for b in psk:
        k3 ^= b
    return bytes([k0, k1, k2, k3])

def decrypt(key4, data):
    out = bytearray()
    prev = 0
    for c in data:
        k = key4[prev & 3]
        idx = (k + prev) & 0xFF
        ks = P2P_PE_TABLE[idx]
        out.append(c ^ ks)
        prev = c
    return bytes(out)

PSK_CANDIDATES = [
    b"685FDC",
    b"camera",
    b"SSD@cs2-network.",
    b"cs2p2p",
    b"",
    b"DGOG",
    b"SHIX",
    b"admin",
    b"1234",
    b"HCAM03247542ABAMS",
    b"check_user",
    b"relay",
    b"p2p",
    b"DGOG-HCAM03247542ABAMS",
    b"685FDC01",
    b"685fdc",
    b"685FDC00",
    b"0000",
]

MARKERS = [b'"pro"', b'"cmd"', b'check_user', b'admin', b'1234', b'"devmac"', b'"user"', b'"pwd"', b'{"']

def extract_relay_payloads(path, network=1):
    pkts = parse_pcap(path)
    payloads = []
    for i, (ts_sec, ts_usec, pkt) in enumerate(pkts):
        if network == 1:
            if len(pkt) < 42:
                continue
            eth_type = struct.unpack_from('>H', pkt, 12)[0]
            if eth_type == 0x0800:
                ip_start = 14
            else:
                continue
        else:
            continue
        
        ip_header_len = (pkt[ip_start] & 0x0F) * 4
        if pkt[ip_start + 9] != 17:
            continue
        udp_start = ip_start + ip_header_len
        payload_start = udp_start + 8
        payload = pkt[payload_start:]
        
        # Look for 00 0d relay header at offset 0
        if len(payload) >= 15 and payload[:2] == b'\x00\x0d' and payload[8:10] == b'\x00\x69':
            # Try various offsets for the encrypted data
            for hdr_len in [15, 16, 24]:
                if len(payload) > hdr_len + 10:
                    ciphertext = payload[hdr_len:]
                    payloads.append({
                        'pkt_idx': i,
                        'hdr_len': hdr_len,
                        'ciphertext': ciphertext,
                        'full_payload': payload
                    })
    return payloads

def brute_force(path):
    payloads = extract_relay_payloads(path, 1)
    print(f"\n{path}: {len(payloads)} relay payloads")
    
    found = []
    for entry in payloads:
        ct = entry['ciphertext']
        for psk in PSK_CANDIDATES:
            key4 = derive_key(psk)
            plaintext = decrypt(key4, ct)
            for marker in MARKERS:
                if marker in plaintext:
                    print(f"\n  *** MATCH ***")
                    print(f"  Pkt {entry['pkt_idx']}, hdr_len={entry['hdr_len']}")
                    print(f"  PSK: {psk}")
                    print(f"  Marker: {marker}")
                    print(f"  Plaintext (first 100 bytes): {plaintext[:100]}")
                    found.append({
                        'pkt_idx': entry['pkt_idx'],
                        'hdr_len': entry['hdr_len'],
                        'psk': psk,
                        'marker': marker,
                        'plaintext': plaintext
                    })
                    break
    return found

for path in [r'E:\open_camera\apk_analysis\capture1_new.pcap',
             r'E:\open_camera\apk_analysis\capture2_new.pcap',
             r'E:\open_camera\apk_analysis\capture3_new.pcap']:
    results = brute_force(path)
    print(f"\n  Total matches: {len(results)}")
