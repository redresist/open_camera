#!/usr/bin/env python3
"""Test decryption of final_capture pkt 4357 with PPPP header at offset 6 - show ALL results."""
import sys
import struct
...
print(f"Relay key: {psk_relay} -> key4: {key4_relay.hex()}")
...
for psk in PSK_CANDIDATES:
    ...
    results.append({'psk': psk, 'ratio': ratio, 'markers': markers, 'pt': plaintext})

# Sort by printable ratio descending
results.sort(key=lambda x: -x['ratio'])

print("\nTop 10 results by printable ratio:")
for i, r in enumerate(results[:10]):
    print(f"\n  #{i+1}: PSK={r['psk']}, ratio={r['ratio']:.2%}, markers={r['markers']}")
    print(f"      First 80 bytes: {r['pt'][:80]}")
    print(f"      Hex: {r['pt'][:80].hex()}")
