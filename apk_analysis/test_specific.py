#!/usr/bin/env python3
"""Test specific PSK combinations on promising packets."""
import sys
import struct

sys.path.insert(0, r'E:\open_camera\apk_analysis')
from parse_pcap import parse_pcap

P2P_PE_TABLE = bytes([])

def derive_key(psk):
    k0 = sum(psk) & 0xFF
    k1 = (-k0) & 0xFF
    k2 = sum(b // 3 for b in psk) & 0xFF
    k3 = 0
    for b in psk:
        k3 ^= b
    return bytes([k0, k1, k2, k3])

def decrypt(key4, data):
    out = bytearray()
    prev = 0
    for c in data:
        k = key4[prev & 3]
        idx = (k + prev) & 0xFF
        ks = P2P_PE_TABLE[idx]
        out.append(c ^ ks)
        prev = c
    return bytes(out)

def test_packet(path, pkt_idx, pppp_offset, extra_psks=None):
    pkts = parse_pcap(path)
    pkt = pkts[pkt_idx][2]
    proto = struct.unpack_from('>H', pkt, 0)[0]
    ip_start = 20
    ip_header_len = (pkt[ip_start] & 0x0F) * 4
    udp_start = ip_start + ip_header_len
    payload = pkt[udp_start+8:]
    
    print(f"\n{'='*60}")
    print(f"{path} pkt {pkt_idx}")
    print(f"Payload len: {len(payload)}")
    print(f"First 32 bytes: {payload[:32].hex()}")
    
    header = payload[pppp_offset:pppp_offset+8]
    print(f"PPPP header at {pppp_offset}: {header.hex()}")
    
    # Try both header interpretations
    for struct_name, rand0_idx, rand1_idx in [
        ("[SIZE][0x68][pad][rand0][rand1][CRC]", 4, 5),
        ("[0x68][pad][rand0][rand1][CRC][SIZE]", 2, 3),
    ]:
        rand0 = header[rand0_idx]
        rand1 = header[rand1_idx]
        psk_relay = f"{rand0:02X}{rand1:02X}".encode()
        key4_relay = derive_key(psk_relay)
        
        ciphertext = payload[pppp_offset+8:]
        layer1 = decrypt(key4_relay, ciphertext)
        
        print(f"\n  Structure: {struct_name}")
        print(f"  Relay key: {psk_relay} -> {key4_relay.hex()}")
        print(f"  Ciphertext len: {len(ciphertext)}")
        print(f"  Layer-1 first 48 bytes hex: {layer1[:48].hex()}")
        
        PSKs = [b"685FDC", b"camera", b"SSD@cs2-network.", b"cs2p2p", b"",
                b"DGOG", b"SHIX", b"admin", b"1234", b"HCAM03247542ABAMS",
                b"check_user", b"relay", b"p2p", b"DGOG-HCAM03247542ABAMS"]
        if extra_psks:
            PSKs.extend(extra_psks)
        
        for psk in PSKs:
            key4 = derive_key(psk)
            pt = decrypt(key4, layer1)
            
            printable = sum(1 for b in pt if 0x20 <= b <= 0x7e or b in (0x0a, 0x0d, 0x00))
            ratio = printable / len(pt)
            
            markers = []
            for marker in [b'"pro"', b'"cmd"', b'check_user', b'admin', b'1234', b'"devmac"', b'"user"', b'"pwd"', b'{"', b'}']:
                if marker in pt:
                    markers.append(marker)
            
            if markers:
                print(f"    PSK={psk}: ratio={ratio:.2%}, markers={markers}")
                print(f"      First 80 bytes: {pt[:80]}")
                print(f"      Hex: {pt[:80].hex()}")

# Test promising candidates
# final_capture.pcap pkt 4357 (0d packet with 68 at offset 6, size_LE=4)
test_packet(r'E:\open_camera\apk_analysis\final_capture.pcap', 4357, 6, [b'5400', b'685FDCcamera', b'685FDC01', b'0000'])

# final_capture.pcap pkt 11 (0d packet with 68 at offset 2)
test_packet(r'E:\open_camera\apk_analysis\final_capture.pcap', 11, 2, [b'0000', b'0100'])

# final_capture.pcap pkt 537 (0d packet with 68 at offset 2)
test_packet(r'E:\open_camera\apk_analysis\final_capture.pcap', 537, 2, [b'0000', b'0200'])
