from scapy.all import rdpcap

shuffle_table = bytes([
    ,
])

def create_psk_hash(psk):
    h = [0, 0, 0, 0]
    for b in psk.encode():
        h[0] = (h[0] + b) & 0xFF
        h[1] = (h[1] - b) & 0xFF
        h[2] = (h[2] + b // 3) & 0xFF
        h[3] = (h[3] ^ b) & 0xFF
    return h

def decrypt(psk_hash, data):
    result = bytearray()
    prev = 0
    for b in data:
        idx = (psk_hash[prev & 3] + prev) & 0xFF
        key = shuffle_table[idx]
        dec = b ^ key
        result.append(dec)
        prev = b
    return bytes(result)

pkts = rdpcap(r'E:\open_camera\apk_analysis\dynamic_analysis\relay_capture.pcap')
uplink = []
for pkt in pkts:
    if hasattr(pkt, 'load'):
        data = bytes(pkt.load)
        if len(data) > 30 and data[0] == 0x00:
            uplink.append(data[15:])

enc = uplink[0]

# Known plaintext candidates
known_prefixes = [
    b'{"pro":"check_user","cmd":100,"devmac":"0000","user":"admin","pwd":"1234"}',
    b'{"pro":"stream","cmd":111,"video":0,"camsmode":0,"user":"admin","pwd":"1234"}',
    b'{"pro":"stream","cmd":111,"video":2,"camsmode":0,"user":"admin","pwd":"1234"}',
]

# Brute force all 65536 possible 2-byte keys
# The hex string is always 4 chars: "%02X%02X"
print("Brute forcing all 65536 keys...")
found = []

for b0 in range(256):
    for b1 in range(256):
        psk = f"{b0:02X}{b1:02X}"
        ph = create_psk_hash(psk)
        dec = decrypt(ph, enc)
        
        for prefix in known_prefixes:
            if dec.startswith(prefix):
                found.append((psk, ph, prefix, dec[:100]))
                print(f"FOUND! PSK={psk}, hash={ph}, prefix={prefix[:50]}...")
                print(f"  DEC: {dec[:100]}")
                break
        
        # Also check for JSON start
        if dec[0] == ord('{') and dec[1] == ord('"') and b'pro' in dec[:20]:
            # More general match
            if b'check_user' in dec or b'stream' in dec or b'admin' in dec:
                found.append((psk, ph, b'GENERAL', dec[:100]))
                print(f"GENERAL MATCH! PSK={psk}, hash={ph}")
                print(f"  DEC: {dec[:100]}")

print(f"\nTotal matches: {len(found)}")
