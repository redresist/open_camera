...
for enc0 in [0x76, 0x77, 0x78]:
    key = enc0 ^ 0x7b
    if key in shuffle_table:
        idx = shuffle_table.index(key)
        print("enc[0]=0x%02x: key=0x%02x, idx=%d" % (enc0, key, idx))
    else:
        print("enc[0]=0x%02x: key=0x%02x NOT IN TABLE" % (enc0, key))

valid_h0 = set(list(range(192, 256)) + list(range(0, 25)))
print("\nValid h[0] count: %d" % len(valid_h0))
valid_keys = set()
for h0 in valid_h0:
    valid_keys.add(shuffle_table[h0])
print("Valid first keystream bytes count: %d" % len(valid_keys))

if 0x0d in valid_keys:
    print("0x0d IS a valid first keystream byte")
else:
    print("0x0d is NOT a valid first keystream byte")

# Show what plaintext first byte we'd get for valid h[0] values with enc0=0x76
print("\nPossible first plaintext bytes for enc[0]=0x76:")
pts = set()
for h0 in valid_h0:
    key = shuffle_table[h0]
    pt = 0x76 ^ key
    pts.add(pt)
print("  Count: %d" % len(pts))
print("  Values: %s" % sorted(pts)[:30])
