#!/usr/bin/env python3
from scapy.all import rdpcap
import struct

shuffle_table = [
    ,
]

shuffle_inverse = [0] * 256
for i, v in enumerate(shuffle_table):
    shuffle_inverse[v] = i

def derive_hash_from_plaintext(enc_data, known_pt):
    h = [None, None, None, None]
    prev = 0
    for i, (eb, pb) in enumerate(zip(enc_data, known_pt)):
        key = eb ^ pb
        key_idx = shuffle_inverse[key]
        slot = prev & 3
        derived = (key_idx - prev) & 0xFF
        if h[slot] is None:
            h[slot] = derived
        elif h[slot] != derived:
            print(f'  CONFLICT at byte {i}: slot={slot}, was {h[slot]}, got {derived}')
            return None
        prev = eb
    return h

def pppp_decrypt_with_hash(psk_hash, data):
    result = []
    prev = 0
    for b in data:
        idx = (psk_hash[prev & 3] + prev) & 0xFF
        key = shuffle_table[idx]
        dec = b ^ key
        result.append(dec)
        prev = b
    return bytes(result)

pkts = rdpcap(r'E:\open_camera\apk_analysis\dynamic_analysis\relay_capture.pcap')

uplink = []
for pkt in pkts:
    if hasattr(pkt, 'load'):
        data = bytes(pkt.load)
        if len(data) > 30 and data[0] == 0x00:
            enc = data[15:]
            uplink.append(enc)

print(f'Found {len(uplink)} uplink packets')
for i, enc in enumerate(uplink[:5]):
    print(f'  Uplink {i}: {len(enc)} bytes: {enc[:40].hex()}')

known_prefixes = [
    b'{"pro":"check_user","cmd":100,"devmac":"0000","user":"admin","pwd":"1234"}',
    b'{"pro":"check_user","cmd":100,"devmac":"0000","user":"admin","pwd":"1234"}\x00',
    b'{"pro":"stream","cmd":111,"video":0,"camsmode":0,"user":"admin","pwd":"1234"}',
    b'{"pro":"stream","cmd":111,"video":0,"camsmode":0,"user":"admin","pwd":"1234"}\x00',
]

for prefix in known_prefixes:
    for enc in uplink[:3]:
        if len(prefix) > len(enc):
            continue
        h = derive_hash_from_plaintext(enc[:len(prefix)], prefix)
        print(f'\nPrefix ({len(prefix)}B): {prefix[:70]}...')
        print(f'  Hash: {h}')
        if h and all(v is not None for v in h):
            dec = pppp_decrypt_with_hash(h, enc)
            print(f'  DECRYPTED: {dec[:100]}')
