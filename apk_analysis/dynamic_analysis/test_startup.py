from scapy.all import rdpcap
import struct
import os

shuffle_table = []

def psk_hash(s):
    h = [0, 0, 0, 0]
    for b in s.encode():
        h[0] = (h[0] + b) & 0xFF
        h[1] = (h[1] - b) & 0xFF
        h[2] = (h[2] + b // 3) & 0xFF
        h[3] = (h[3] ^ b) & 0xFF
    return h

def decrypt_with_hash(h, data):
    out = bytearray()
    prev = 0
    for b in data:
        idx = (h[prev & 3] + prev) & 0xFF
        key = shuffle_table[idx]
        dec = b ^ key
        out.append(dec)
        prev = b
    return bytes(out)

with open(r"E:\open_camera\apk_analysis\dynamic_analysis\startup_test.txt", "w", encoding="utf-8") as f:
    for name in ['startup.pcap', 'capture1_new.pcap', 'relay_capture.pcap']:
        if name == 'relay_capture.pcap':
            path = os.path.join(r"E:\open_camera\apk_analysis\dynamic_analysis", name)
        else:
            path = os.path.join(r"E:\open_camera\apk_analysis", name)
        
        try:
            pkts = rdpcap(path)
            f.write(f"\n=== {name} ===\n")
            for i, pkt in enumerate(pkts):
                if not hasattr(pkt, "load"):
                    continue
                data = bytes(pkt.load)
                if len(data) > 30 and data[0] == 0x00:
                    enc = data[15:]
                    h = psk_hash("camera")
                    dec = decrypt_with_hash(h, enc)
                    safe = ''.join(chr(b) if 32 <= b < 127 else '.' for b in dec)
                    f.write(f"Pkt {i}: len={len(data)} enc[0]=0x{enc[0]:02x} dec[0]=0x{dec[0]:02x}\n")
                    f.write(f"  hex: {dec[:40].hex()}\n")
                    f.write(f"  safe: {safe[:60]}\n")
                    break
        except Exception as e:
            f.write(f"{name}: error: {e}\n")

print("Results written to startup_test.txt")
