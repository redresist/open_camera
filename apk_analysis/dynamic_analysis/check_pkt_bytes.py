shuffle_table = []

valid_h0 = set(list(range(192, 256)) + list(range(0, 25)))

for enc0 in [0x64, 0xb5]:
    key_for_brace = enc0 ^ 0x7b
    print(f"\nenc[0]=0x{enc0:02x}:")
    print(f"  Key for '{{' (0x7b): 0x{key_for_brace:02x}")
    if key_for_brace in shuffle_table:
        idx = shuffle_table.index(key_for_brace)
        print(f"  idx={idx}, valid h0={idx in valid_h0}")
    else:
        print(f"  NOT IN TABLE")
    
    # Show all possible first plaintext chars
    pts = {}
    for h0 in valid_h0:
        key = shuffle_table[h0]
        pt = enc0 ^ key
        if pt not in pts:
            pts[pt] = []
        pts[pt].append(h0)
    
    print(f"  Possible first plaintext bytes ({len(pts)} unique):")
    for pt in sorted(pts.keys())[:20]:
        char = chr(pt) if 32 <= pt < 127 else f"0x{pt:02x}"
        print(f"    {char} (0x{pt:02x}): h0={pts[pt][:5]}")
