...
shuffle_inverse = {v: i for i, v in enumerate(shuffle_table)}
valid_h0 = set(list(range(192, 256)) + list(range(0, 25)))

# Test the hypothesis that plaintext starts with the direct_relay header
# header = bytes.fromhex("000d000100000000000000000101")
direct_header = bytes.fromhex("000d000100000000000000000101")

captures = {
    'capture1_new.pcap': 0x76,
    'capture2_new.pcap': 0xb9,
    'capture3_new.pcap': 0xe9,
    'relay_pkt5': 0x64,
    'relay_pkt13': 0xb5,
}

for name, enc0 in captures.items():
    # If plaintext[0] = 0x00 (first byte of header)
    h0_00 = shuffle_inverse.get(enc0 ^ 0x00)
    valid_00 = h0_00 in valid_h0 if h0_00 is not None else False
    
    # If plaintext[0] = 0x0d (second byte of header pattern)
    h0_0d = shuffle_inverse.get(enc0 ^ 0x0d)
    valid_0d = h0_0d in valid_h0 if h0_0d is not None else False
    
    print(f"{name}: enc0=0x{enc0:02x}")
    print(f"  pt=0x00 -> h0={h0_00}, valid={valid_00}")
    print(f"  pt=0x0d -> h0={h0_0d}, valid={valid_0d}")
    
    # Check if first byte of direct_relay header matches
    for pt0 in [0x00, 0x0d]:
        h0 = shuffle_inverse.get(enc0 ^ pt0)
        if h0 in valid_h0:
            print(f"  >>> VALID: pt[0]=0x{pt0:02x}, h0={h0}")
