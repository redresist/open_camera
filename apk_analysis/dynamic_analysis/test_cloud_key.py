from scapy.all import rdpcap
import struct
import sys

shuffle_table = []

def psk_hash(s):
    h = [0, 0, 0, 0]
    for b in s.encode():
        h[0] = (h[0] + b) & 0xFF
        h[1] = (h[1] - b) & 0xFF
        h[2] = (h[2] + b // 3) & 0xFF
        h[3] = (h[3] ^ b) & 0xFF
    return h

def decrypt_with_hash(h, data):
    out = bytearray()
    prev = 0
    for b in data:
        idx = (h[prev & 3] + prev) & 0xFF
        key = shuffle_table[idx]
        dec = b ^ key
        out.append(dec)
        prev = b
    return bytes(out)

def safe_text(data):
    return ''.join(chr(b) if 32 <= b < 127 else '.' for b in data)

pkts = rdpcap(r"E:\open_camera\apk_analysis\dynamic_analysis\relay_capture.pcap")

# Get encrypted payloads from 0x00 packets
payloads = []
for i, pkt in enumerate(pkts):
    if not hasattr(pkt, "load"):
        continue
    data = bytes(pkt.load)
    if len(data) > 30 and data[0] == 0x00:
        enc = data[15:]
        payloads.append((i, enc))

# Write output to file
with open(r"E:\open_camera\apk_analysis\dynamic_analysis\decrypt_tests.txt", "w", encoding="utf-8") as f:
    f.write(f"Found {len(payloads)} uplink packets\n\n")
    
    candidates = [
        ("685F", "hex of first 2 bytes"),
        ("685FDC", "full hex string"),
        ("3638", "ascii of '68'"),
        ("363835", "ascii of '685'"),
        ("363835464443", "ascii of '685FDC'"),
        ("68", "just first byte"),
        ("685FDC00", "hex padded"),
        ("camera", "default"),
    ]
    
    for psk, desc in candidates:
        h = psk_hash(psk)
        f.write(f"=== PSK='{psk}' ({desc}) hash={h} ===\n")
        for pkt_idx, enc in payloads:
            dec = decrypt_with_hash(h, enc)
            text = safe_text(dec)
            f.write(f"  Pkt {pkt_idx}: {text[:80]}\n")
            f.write(f"         hex: {dec[:40].hex()}\n")
        f.write("\n")

print("Results written to decrypt_tests.txt")
