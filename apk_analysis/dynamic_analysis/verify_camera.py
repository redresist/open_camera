from scapy.all import rdpcap
import struct

shuffle_table = []

def psk_hash(s):
    h = [0, 0, 0, 0]
    for b in s.encode():
        h[0] = (h[0] + b) & 0xFF
        h[1] = (h[1] - b) & 0xFF
        h[2] = (h[2] + b // 3) & 0xFF
        h[3] = (h[3] ^ b) & 0xFF
    return h

def decrypt_with_hash(h, data):
    out = bytearray()
    prev = 0
    for b in data:
        idx = (h[prev & 3] + prev) & 0xFF
        key = shuffle_table[idx]
        dec = b ^ key
        out.append(dec)
        prev = b
    return bytes(out)

# Test capture1_new.pcap with camera
pkts = rdpcap(r"E:\open_camera\apk_analysis\capture1_new.pcap")
h = psk_hash("camera")
print(f"PSK='camera' hash={h}")

for i, pkt in enumerate(pkts):
    if not hasattr(pkt, "load"):
        continue
    data = bytes(pkt.load)
    if len(data) > 30 and data[0] == 0x00:
        enc = data[15:]
        dec = decrypt_with_hash(h, enc)
        print(f"Pkt {i}: enc[0]=0x{enc[0]:02x} dec[0]=0x{dec[0]:02x} text={dec[:50]}")
        break
