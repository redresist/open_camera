#!/usr/bin/env python3
"""Test encrypted LAN search - burst then listen."""
import socket
import struct

SHUFFLE = bytes([
    ,
])
KEY = [0x69, 0x97, 0xcc, 0x19]

def enc(data):
    out, prev = bytearray(), 0
    for b in data:
        idx = (KEY[prev & 3] + prev) & 0xFF
        e = b ^ SHUFFLE[idx]
        out.append(e); prev = e
    return bytes(out)

def dec(data):
    out, prev = bytearray(), 0
    for b in data:
        idx = (KEY[prev & 3] + prev) & 0xFF
        out.append(b ^ SHUFFLE[idx])
        prev = b
    return bytes(out)

probe = enc(bytes([0xf1, 0x30, 0x00, 0x00]))
print(f"Probe: {probe.hex()} (match 2cba5f5d: {probe.hex() == '2cba5f5d'})")

s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
s.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, 1)
s.bind(("192.168.1.100", 0))
s.settimeout(3)

# Burst send to all targets
for cam in ["192.168.1.133", "192.168.1.148", "255.255.255.255"]:
    for port in [32108, 32100]:
        s.sendto(probe, (cam, port))

# Listen for responses
print("Listening...")
seen = set()
while True:
    try:
        data, addr = s.recvfrom(4096)
        if addr not in seen:
            seen.add(addr)
            ptype = dec(data)[1]
            print(f"  {addr}: {data[:20].hex()} type=0x{ptype:02x}")
    except socket.timeout:
        break
s.close()
print(f"Done. {len(seen)} responders.")
