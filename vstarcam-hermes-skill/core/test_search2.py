#!/usr/bin/env python3
"""Test BOTH encrypted (2c) and unencrypted (f1) probes on port 32108."""
import socket

SHUFFLE = bytes([])
KEY = [0x69, 0x97, 0xcc, 0x19]

def enc(data):
    out, prev = bytearray(), 0
    for b in data:
        idx = (KEY[prev & 3] + prev) & 0xFF
        e = b ^ SHUFFLE[idx]; out.append(e); prev = e
    return bytes(out)

def dec(data):
    out, prev = bytearray(), 0
    for b in data:
        idx = (KEY[prev & 3] + prev) & 0xFF
        out.append(b ^ SHUFFLE[idx]); prev = b
    return bytes(out)

probes = {
    "encrypted (2c)": enc(bytes([0xf1, 0x30, 0x00, 0x00])),
    "unencrypted (f1)": bytes([0xf1, 0x30, 0x00, 0x00]),
    "ext unencrypted (f1)": bytes([0xf1, 0x32, 0x00, 0x00]),
}

s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
s.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, 1)
s.bind(("0.0.0.0", 0))
s.settimeout(4)

for name, probe in probes.items():
    for cam in ["192.168.1.133", "192.168.1.148", "255.255.255.255"]:
        for port in [32108, 32100]:
            s.sendto(probe, (cam, port))

print("Listening for 4s...")
seen = set()
while True:
    try:
        data, addr = s.recvfrom(4096)
        if addr not in seen:
            seen.add(addr)
            try:
                d = dec(data) if data[0] == 0x2c else data
                if len(d) >= 4:
                    magic = d[0]; op = d[1]; plen = (d[2] << 8) | d[3]
                    payload = d[4:4+plen] if plen > 0 else b""
                    uid_info = ""
                    if op == 0x41 and len(payload) >= 20:
                        pref = payload[:8].decode("ascii",errors="replace").rstrip("\x00")
                        ser = int.from_bytes(payload[8:12],"big")
                        ck = payload[12:20].decode("ascii",errors="replace").rstrip("\x00")
                        uid_info = f" {pref}-{ser:06d}-{ck}"
                    print(f"  {addr}: magic=0x{magic:02x} op=0x{op:02x} len={plen}{uid_info} ({data[:24].hex()})")
            except:
                print(f"  {addr}: {data[:24].hex()}")
    except socket.timeout:
        break
s.close()
print(f"Done. {len(seen)} unique responders.")
